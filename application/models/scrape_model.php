...
class Scrape_model extends CI_Model {
    function __construct() {
        ...
        $this->field = array( 'id', 'category_id', 'title', 'index_link', 'last_update' );
    }
    ...
    function get_by_id($param) {
        ...
        if (isset($param['id'])) {
            $select_query  = "SELECT * FROM ".SCRAPE." WHERE id = '".$param['id']."' LIMIT 1";
        } else if (isset($param['older'])) {
            $select_query  = "SELECT * FROM ".SCRAPE." ORDER BY last_update ASC LIMIT 1";
        } 
    ...
    function get_array($param = array()) {
		...
		$select_query = "
			SELECT SQL_CALC_FOUND_ROWS SCRAPE.*
			FROM ".SCRAPE." SCRAPE
